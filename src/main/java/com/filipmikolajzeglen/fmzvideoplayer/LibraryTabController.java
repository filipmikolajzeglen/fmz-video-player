...
public class LibraryTabController
{
   ...
   @FXML
   private ScrollPane libraryScrollPane;

   @FXML
   private AnchorPane libraryContent;

   private StartupConfigController startupConfigController;

   @FXML
   public void initialize() {
      // Zarejestruj siebie w properties, aby StartupConfigController mógł pobrać kontroler
      libraryContent.getProperties().put("controller", this);
   }

   public void setStartupConfigController(StartupConfigController controller) {
      this.startupConfigController = controller;
   }

   public void refreshLibrary(String basePath, List<SeriesInfo> series) {
   ...
   private void showSeriesDetailView(String basePath, SeriesInfo seriesInfo)
   {
      ...
      playAllButton.setOnAction(event -> {
         ...
         if (startupConfigController != null) {
            startupConfigController.onPlayClicked();
         }
      });
