...
class CommercialsManager
{
   private List<String> commercialPaths = new ArrayList<>();

   void loadCommercials()
   {
      String commercialFolderPath = FMZVideoPlayerConfiguration.Paths.VIDEO_MAIN_SOURCE + File.separator
            + FMZVideoPlayerConfiguration.Paths.COMMERCIALS_FOLDER_NAME;
   ...
   List<String> getRandomCommercialPlaylist()
   {
      ...
      return shuffledAds.stream()
            .limit(FMZVideoPlayerConfiguration.Playback.COMMERCIAL_COUNT_BETWEEN_EPISODES)
            .collect(Collectors.toList());
   }

   /**
    * Decyduje, czy należy odtworzyć blok reklamowy i zwraca listę reklam, jeśli tak.
    * @param hasNextEpisode Informacja, czy w playliście jest jeszcze następny odcinek.
    * @return Optional z listą ścieżek do reklam lub pusty Optional, jeśli reklamy nie powinny być odtwarzane.
    */
   public Optional<List<String>> getCommercialsToPlay(boolean hasNextEpisode)
   {
      if (FMZVideoPlayerConfiguration.Playback.COMMERCIALS_ENABLED && hasNextEpisode)
      {
