...
public class VideoPlayer implements Initializable
{
   ...
   private void initializeMediaPlayer(String videoPath)
   {
      ...
      {
         private int sampleMagnitudeCount = 0;

         @Override
         public void spectrumDataUpdate(double timestamp, double duration, float[] magnitudes, float[] phases)
         {
            mediaPlayer.setVolume(0.65);
            if (currentVideo.getAudioNormalizedVolume() != null)
            {
               mediaPlayer.setVolume(currentVideo.getAudioNormalizedVolume());
               LOGGER.info(String.format(
                     "Normalize audio volume for: %s - %s was set to: %f.",
                     currentVideo.getSeriesName(), currentVideo.getEpisodeName(),
                     currentVideo.getAudioNormalizedVolume())
               );
            ...
            else
            {
               ...
               averageMagnitude /= magnitudes.length;
               sampleMagnitudeCount++;

               double targetVolume = 0.5;
               double logBase = 10.0;
               double decibelScaleFactor = 20.0;
               double correctionFactor = targetVolume / Math.pow(logBase, averageMagnitude / decibelScaleFactor);
               double normalizedVolume = ((targetVolume * correctionFactor) / 1000) + targetVolume;
               mediaPlayer.setVolume(normalizedVolume);

               if (sampleMagnitudeCount >= 100)
               {
                  currentVideo.setAudioNormalizedVolume(normalizedVolume);
                  videoPlayerService.getDatabase().save(currentVideo);

                  mediaPlayer.setVolume(normalizedVolume);
                  LOGGER.info(String.format(
                        "Normalize audio volume for: %s - %s was saved and set to: %f. Final Average Magnitude: %f",
                        currentVideo.getSeriesName(), currentVideo.getEpisodeName(), normalizedVolume,
                        averageMagnitude)
                  );
                  mediaPlayer.setAudioSpectrumListener(null);
               }
   ...
   /**
    * Binds the text of the current time label to the current time of the video. This method allows the timer to update
    * in synchrony with the video. If the mediaPlayer is NULL, an error message is logged.
    * <p>
    * Binding created using the createStringBinding method to invoke a Callable, which is responsible for returning the
    * formatted time string with hours, minutes and seconds. The time is expressed in milliseconds e.g., 750.0 ms.
    */
   ...
   {
      setSvgGraphic(buttonPlayPauseRestartSVG, VideoPlayerConfiguration.PLAY_SVG,
            VideoPlayerConfiguration.PLAY_SVG_SCALE_MODIFIER);
      buttonPlayPauseRestart.setGraphic(buttonPlayPauseRestartSVG);
   }

   private void setButtonPauseSVG()
   {
      setSvgGraphic(buttonPlayPauseRestartSVG, VideoPlayerConfiguration.PAUSE_SVG,
            VideoPlayerConfiguration.PAUSE_SVG_SCALE_MODIFIER);
      buttonPlayPauseRestart.setGraphic(buttonPlayPauseRestartSVG);
   }

   private void setButtonRestartSVG()
   {
      setSvgGraphic(buttonPlayPauseRestartSVG, VideoPlayerConfiguration.RESTART_SVG,
            VideoPlayerConfiguration.RESTART_SVG_SCALE_MODIFIER);
      buttonPlayPauseRestart.setGraphic(buttonPlayPauseRestartSVG);
   }
   ...
   {
      setSvgGraphic(labelFullScreenSVG, VideoPlayerConfiguration.FULLSCREEN_SVG,
            VideoPlayerConfiguration.SCREEN_MODE_SVG_SCALE_MODIFIER);
      labelFullScreen.setGraphic(labelFullScreenSVG);
   }

   private void setLabelExitFullscreenSVG()
   {
      setSvgGraphic(labelFullScreenSVG, VideoPlayerConfiguration.EXIT_SVG,
            VideoPlayerConfiguration.SCREEN_MODE_SVG_SCALE_MODIFIER);
      labelFullScreen.setGraphic(labelFullScreenSVG);
   }
   ...
   {
      setSvgGraphic(labelVolumeSVG, VideoPlayerConfiguration.VOLUME1_SVG,
            VideoPlayerConfiguration.VOLUME_SVG_SCALE_MODIFIER);
      labelVolume.setGraphic(labelVolumeSVG);
   }

   private void setLabelVolume2SVG()
   {
      setSvgGraphic(labelVolumeSVG, VideoPlayerConfiguration.VOLUME2_SVG,
            VideoPlayerConfiguration.VOLUME_SVG_SCALE_MODIFIER);
      labelVolume.setGraphic(labelVolumeSVG);
   }
   ...
   {
      setSvgGraphic(labelVolumeSVG, VideoPlayerConfiguration.MUTE_SVG,
            VideoPlayerConfiguration.VOLUME_SVG_SCALE_MODIFIER);
      labelVolume.setGraphic(labelVolumeSVG);
   }
